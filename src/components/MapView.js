...
const MapView = ({ filteredItems, itemToshow }) => {
  ...
  useEffect(() => {
    if (Object?.keys(itemToshow)?.length !== 0) {
      ...
      setMarkers([
        {
          position: [itemToshow?.location?.lat, itemToshow?.location?.lng],
          name: itemToshow?.name,
          description: itemToshow?.summary,
          property_type: itemToshow?.property_type,
          rent_range: itemToshow?.rent_range,
        },
      ]);
